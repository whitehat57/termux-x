...
ui_intro() {
  ...
  printf '%b├────────────────────────────────────────────┤%b\n' "$TX_CYAN" "$TX_RESET"
  printf '%b│%b Cyberpunk mobile workstation for Termux.  %b│%b\n' "$TX_CYAN" "$TX_RESET" "$TX_CYAN" "$TX_RESET"
  printf '%b│%b Mode      : Automatic wizard              %b│%b\n' "$TX_CYAN" "$TX_RESET" "$TX_CYAN" "$TX_RESET"
  printf '%b│%b Focus     : UI, Zsh, runtime, CLI tools    %b│%b\n' "$TX_CYAN" "$TX_RESET" "$TX_CYAN" "$TX_RESET"
...
ui_finish() {
  ...
  printf '%b│%b Workstation : Python + Go + Node.js       %b│%b\n' "$TX_CYAN" "$TX_RESET" "$TX_CYAN" "$TX_RESET"
  printf '%b│%b File Tools  : Ranger + nnn + tree         %b│%b\n' "$TX_CYAN" "$TX_RESET" "$TX_CYAN" "$TX_RESET"
  printf '%b│%b Extra Keys  : Termux default              %b│%b\n' "$TX_CYAN" "$TX_RESET" "$TX_CYAN" "$TX_RESET"
  printf '%b╰────────────────────────────────────────────╯%b\n' "$TX_CYAN" "$TX_RESET"
  printf '\n'
  printf '%bLog file:%b %s\n' "$TX_DIM" "$TX_RESET" "$log_file"
  printf '%bRestart Termux to enjoy the new workstation.%b\n' "$TX_GREEN" "$TX_RESET"
}
